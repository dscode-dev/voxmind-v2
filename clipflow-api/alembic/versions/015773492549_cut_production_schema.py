...
Nineteen tables, and every one of them is on the path a video takes: the operator, what the
automation finds, the runs that turn a find into cuts, what those cuts become on the channel,
and what the channel says about them afterwards.

There is no upgrade path from the old chain, deliberately and with the owner's agreement: the
database is recreated.

Revision ID: 015773492549
Revises:
...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_automation_states_topic_id'), 'automation_states', ['topic_id'], unique=True)
